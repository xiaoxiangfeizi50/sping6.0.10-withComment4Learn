...
public class DefaultAdvisorChainFactory implements AdvisorChainFactory, Serializable {
	...
	public List<Object> getInterceptorsAndDynamicInterceptionAdvice(
		...
		// 这里用了一个单例模式【getInstance()】 获取DefaultAdvisorAdapterRegistry实例
		...
		/**  此处返回三个适配器，before、returning、throwing，没有around和after ,问why？ 点进去看DefaultAdvisorAdapterRegistry的构造器上答案 */
		AdvisorAdapterRegistry registry = GlobalAdvisorAdapterRegistry.getInstance();
		Advisor[] advisors = config.getAdvisors();// 此处获取六个通知，exposed、以及before/around/after/returning/throwing
		...
		for (Advisor advisor : advisors) {
			// 如果是PointcutAdvisor类型的实例，开始二次匹配
			if (advisor instanceof PointcutAdvisor pointcutAdvisor) {
				// Add it conditionally.
				// 如果提前进行过切点的匹配了或者当前的Advisor适用于目标类
				if (config.isPreFiltered() || pointcutAdvisor.getPointcut().getClassFilter().matches(actualClass)) {
					// 前面MethodMatcher进行表达式匹配的时候，类只要有一个方法有匹配就放到了·此处进行二次匹配，用来匹配具体的方法
