...
public class ClassPathBeanDefinitionScanner extends ClassPathScanningCandidateComponentProvider {
	...
	public ClassPathBeanDefinitionScanner(BeanDefinitionRegistry registry, boolean useDefaultFilters,
		...
		if (useDefaultFilters) {
			registerDefaultFilters(); // 注册默认注解过滤器@Component等
		}
	...
	public int scan(String... basePackages) {
		int beanCountAtScanStart = this.registry.getBeanDefinitionCount(); // 获取容器中已经注册的bean数量，刚开始只能获取到一些核心的处理器
		// 开始扫描，扫描完会进行注册（不会依赖注入，后置处理器也不会执行，包括Aware）
		doScan(basePackages);

		// 如果配置了排除部分bean，则在去注册一边内置Bean的
	...
	protected Set<BeanDefinitionHolder> doScan(String... basePackages) {
		System.out.println("--------------------开始进行包扫描：" + Arrays.toString(basePackages));
		...
		for (String basePackage : basePackages) {
			...
			for (BeanDefinition candidate : candidates) {
				ScopeMetadata scopeMetadata = this.scopeMetadataResolver.resolveScopeMetadata(candidate); // 解析bean定义作用域
				candidate.setScope(scopeMetadata.getScopeName());
				String beanName = this.beanNameGenerator.generateBeanName(candidate, this.registry);
				// 判断如果实现了BeanDefinitionRegistryPostProcessor，标识一些额外的属性（设置一些默认值等）
