...
public class ClassPathScanningCandidateComponentProvider implements EnvironmentCapable, ResourceLoaderAware {
	...
	public Set<BeanDefinition> findCandidateComponents(String basePackage) {
		if (this.componentsIndex != null && indexSupportsIncludeFilters()) { // 判断是否已经建立了组件索引，如果建立了就不用包扫描
			return addCandidateComponentsFromIndex(this.componentsIndex, basePackage);
		}
		else {
			return scanCandidateComponents(basePackage); // 进行包扫描
		}
	}
	...
	private Set<BeanDefinition> scanCandidateComponents(String basePackage) {
		Set<BeanDefinition> candidates = new LinkedHashSet<>(); // 开始包扫描
		try {
			String packageSearchPath = ResourcePatternResolver.CLASSPATH_ALL_URL_PREFIX +
					resolveBasePackage(basePackage) + '/' + this.resourcePattern; // 算出需要扫描的路径
			Resource[] resources = getResourcePatternResolver().getResources(packageSearchPath); // 获取包下所有的类文件
			boolean traceEnabled = logger.isTraceEnabled();
			boolean debugEnabled = logger.isDebugEnabled();
			for (Resource resource : resources) {
				...
				try {// 根据类文件信息和类加载器，读取到元信息（类名，方法名，注解等）
					MetadataReader metadataReader = getMetadataReaderFactory().getMetadataReader(resource);
					if (isCandidateComponent(metadataReader)) { // scann注解的排除属性过滤
						// 元信息包装成BeanDefinition
						ScannedGenericBeanDefinition sbd = new ScannedGenericBeanDefinition(metadataReader);
						sbd.setSource(resource);
						System.out.println("-------------------通过包获取到了类信息MetadataReader，封装成BeanDefinition：" + sbd.getBeanClassName());
						if (isCandidateComponent(sbd)) {
							if (debugEnabled) {
