...
public class DataSourceTransactionManager extends AbstractPlatformTransactionManager
	...
	protected Object doGetTransaction() {
		...
		// 从事务管理器中获取一个ConnectionHolder，方便后面获取数据库连接
	...
	protected void doBegin(Object transaction, TransactionDefinition definition) {
		...
		try {
			if (!txObject.hasConnectionHolder() ||
				...
				System.out.println("从数据库获取连接" + newCon.getCatalog());
			...
			if (con.getAutoCommit()) {
				...
				// 会在数据库中执行这个命令SET autocommit=0
				System.out.println("设置手动提交事务：" + con.getCatalog());
				con.setAutoCommit(false);
			}

			prepareTransactionalConnection(con, definition);
			txObject.getConnectionHolder().setTransactionActive(true); // 设置事务活跃状态
			// 判断是否设置了事务超时时间，如果设置了，就设置到连接持有者中
			int timeout = determineTimeout(definition);
			if (timeout != TransactionDefinition.TIMEOUT_DEFAULT) {
				txObject.getConnectionHolder().setTimeoutInSeconds(timeout);
			}

			// 如果是新的连接持有者，就绑定到事务管理器中
	...
	/**
	 * 判断事务是否使用只读事务，如果是的话执行SET TRANSACTION READ ONLY语句，Prepare the transactional {@code Connection} right after transaction begin.
