...
public abstract class TransactionAspectSupport implements BeanFactoryAware, InitializingBean {
	...
	/**
	 * 事务操作核心代码之一，创建数据库连接，事务的隔离级别，提交事务，回滚事务等，General delegate for around-advice-based subclasses, delegating to several other template
	...
	protected Object invokeWithinTransaction(Method method, @Nullable Class<?> targetClass,
		...
		// 获取事务属性，之前就解析过属性注解，就是个这玩意PROPAGATION_REQUIRED,ISOLATION_DEFAULT，事务的传播行为和隔离级别，根据这个去创建事务管理器
		...
		if (txAttr == null || !(ptm instanceof CallbackPreferringPlatformTransactionManager cpptm)) {
			...
			Object retVal;
			try {
				// 执行下一个拦截器，说白了就开始执行目标方法了，到这里已经创建事务成功了
				...
				// 清理当前线程的事务信息，方便下一次使用（一个线程运行开启多个事务）
				cleanupTransactionInfo(txInfo);
			}
			// 如果目标方法执行成功并且返回值是 Vavr Try 类型，调用evaluateTryFailure根据事务回滚规则，来回滚事务
		...
		// 如果事务管理器是CallbackPreferringPlatformTransactionManager类型，则可以在事务中执行一些回调函数TransactionCallback（比如在事务提交前或者事务回滚前执行某些操作，清理资源发送消息）
		else {
			Object result;
