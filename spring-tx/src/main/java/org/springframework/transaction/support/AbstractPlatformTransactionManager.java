...
public abstract class AbstractPlatformTransactionManager implements PlatformTransactionManager, Serializable {
	...
	public final TransactionStatus getTransaction(@Nullable TransactionDefinition definition)
		...
		// 创建事务定义，如果传入的事务定义为null，则使用默认的事务定义
		TransactionDefinition def = (definition != null ? definition : TransactionDefinition.withDefaults());
		// 创建事务连接对象维护ConnectionHolder，用于保存数据库连接
		Object transaction = doGetTransaction();
		boolean debugEnabled = logger.isDebugEnabled();
		// 检查数据库连接是否处于活动状态，判断外层事务正在运行
		...
		// 事务的隔离级别是否为PROPAGATION_MANDATORY，抛出异常
		if (def.getPropagationBehavior() == TransactionDefinition.PROPAGATION_MANDATORY) {
			throw new IllegalTransactionStateException(
					"No existing transaction found for transaction marked with propagation 'mandatory'");
		}
		// 或者是PROPAGATION_REQUIRED，PROPAGATION_REQUIRES_NEW，PROPAGATION_NESTED，则会进行开启事务
			...
			catch (RuntimeException | Error ex) { // 如果出错则会恢复挂起
	...
	private TransactionStatus startTransaction(TransactionDefinition definition, Object transaction,
		...
		// 创建事务状态，这里就是正常的手动开启事务的逻辑代码
		DefaultTransactionStatus status = newTransactionStatus(
				definition, transaction, true, newSynchronization, debugEnabled, suspendedResources);
		// 开启事务
		doBegin(transaction, definition);
		// 初始化事务同步器（如果是新的事务，标记活动状态，设置当前事务隔离级别，只读和事务名称，当事务提交回滚，事务同步器会触发响应操作）
	...
	private TransactionStatus handleExistingTransaction(
			...
			throws TransactionException {
		// 判断事务隔离级别是否为NEVER，不开启新的事务直接抛出异常
		if (definition.getPropagationBehavior() == TransactionDefinition.PROPAGATION_NEVER) {
			throw new IllegalTransactionStateException(
					"Existing transaction found for transaction marked with propagation 'never'");
		}
		// 判断事务隔离级别NOT_SUPPORTED，是则进行挂起事务（暂存）
		...
		// 判断事务隔离级别是否为REQUIRES_NEW，是则进行挂起并且开启新的事务（会走这里，因为配置了PROPAGATION_REQUIRES_NEW）
		...
		// 判断事务隔离级别是否为PROPAGATION_NESTED，融合到外层事务中，SavePoint机制，外层影响内层，内层不会影响外层
	...
	/**
	 * 创建并准备一个新的事务状态，包含事务状态，事务定义，是否新事务，是否需要新的同步，日志，以及挂起资源
	 *
	...
	/**
	 * 创建事务状态对象，维护一些默认的事务属性，Create a TransactionStatus instance for the given arguments.
	 */
	...
	/**
	 * 判断是否挂起事务，需要则挂起，Suspend the given transaction. Suspends transaction synchronization first,
	...
	protected final SuspendedResourcesHolder suspend(@Nullable Object transaction) throws TransactionException {
		// 检查当前是否有活跃的事务同步，外层事务是否正在活动（事务同步机制，讲事务和数据库连接等资源关联起来）
		if (TransactionSynchronizationManager.isSynchronizationActive()) {
			...
			try {
				...
				if (transaction != null) {
					// 挂起操作，就是清空当前事务资源ConnectionHolder，并返回
					suspendedResources = doSuspend(transaction);
					System.out.println("挂起事务：" + transaction);
				}
				// 获取挂起的事务信息
				...
				// 暂存在一个SuspendedResourcesHolder对象中
	...
	protected final void resume(@Nullable Object transaction, @Nullable SuspendedResourcesHolder resourcesHolder)
		...
		if (resourcesHolder != null) {
			...
			if (suspendedResources != null) {
				// 恢复挂起，跟挂起一样维护map，put回去，挂起只不过是remove
				doResume(transaction, suspendedResources);
				System.out.println("恢复挂起的事务："+transaction);
			}
	...
	/**
	 * 挂起所有的事务同步，Suspend all current synchronizations and deactivate transaction
	...
	private void processCommit(DefaultTransactionStatus status) throws TransactionException {
		try {
			...
			try {
				...
				// 事务提交前的准备操作，空实现可自行重写这个方法（例如检查事务状态或进行某些清理工作）
				prepareForCommit(status);
				// 触发事务提交前的回调AbstractPlatformTransactionManager（事务同步器是Spring提供可以在事务各个阶段完成一些钩子函数）
				triggerBeforeCommit(status);
				// 触发事务完成前的回调triggerBeforeCompletion，提交前还是回滚前都会执行这个回调（事务同步器是Spring提供可以在事务各个阶段完成一些钩子函数）
				...
				// 如果事务标记为全局回滚，则将unexpectedRollback设置成true
